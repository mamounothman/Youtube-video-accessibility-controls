...
var requirementsTxt = "You need Flash player 8+ and JavaScript enabled to view this video.";
var headerTxt = "Player Controls";
var playTxt = "Play";
var forwardTxt = "Forward 20%";
var backTxt = "Back 20%";
var stopTxt = "Stop";
var volUpTxt = "Volume Up";
var volDnTxt = "Volume Down";
var muteTxt = "Mute";
var loopTxt = "Loop";
var currentlyPlayingTxt = "Currently Playing:";
var timeTxt = "Time:";
var unmuteTxt = "Unmute";
var pauseTxt = "Pause";
var unloopTxt = "UnLoop";
var videoPlayListTxt = "Video Play List";
var ofTxt = "of";
    ...
    for (var i=0; i<ytp.childNodes.length; i++)
    {
      if (ytp.childNodes[i].className) 
      {
        if (movpattern.test(ytp.childNodes[i].className)) 
        {
          ...
          var yttext = '';
          if (ytp.childNodes[i].firstChild != null) {
            yttext = ytTrim(ytp.childNodes[i].firstChild.nodeValue);
          }
          var ytlistobj = {
            url : yturl,
            text : yttext
...
function updateButtonState(ytpid)
{
  ...
  { 
    ...
    if (ytp.isMuted()) 
    {
      mutebut.firstChild.nodeValue = Drupal.settings.YTP.unmuteTxt;
    }
    else 
    {
      mutebut.firstChild.nodeValue = Drupal.settings.YTP.muteTxt;
    }
    if (ytp.getPlayerState() == 1)
    {
      playbut.firstChild.nodeValue = Drupal.settings.YTP.pauseTxt;
    }
    if (ytp.getPlayerState() == 2)
    {
      playbut.firstChild.nodeValue = Drupal.settings.YTP.playTxt
    }
  }
}
function prepTime(s)
...
function updateTime(ytpid)
{
  ...
  if (new RegExp('^(' + ytplayer.join('|') + ')$').test(ytp.id))
  {
    ...
    var timeval = document.createTextNode(prepTime(Math.round(ytp.getCurrentTime())) +" "+ofTxt+" "+ prepTime(Math.round(ytp.getDuration())));
...
function ytloop(ytpid)
{
  ...
  if (new RegExp('^(' + ytplayer.join('|') + ')$').test(ytp.id)) 
  {
  	var loopbut = document.getElementById("ytloopbut"+ytpid);
   	if (loopbut.firstChild.nodeValue == Drupal.settings.YTP.loopTxt)
    {
    	loopbut.firstChild.nodeValue = Drupal.settings.YTP.unloopTxt;
      ytLoopInterval[ytpid] = window.setInterval(function() { ytp.playVideo(); }, 2000);
    }
    else
    {
    	loopbut.firstChild.nodeValue = Drupal.settings.YTP.loopTxt;
...
function ytPlayerLoad(ytmovurl,width,pid)
{
  ...
  var ytparams = {
    ...
    cc_load_policy: "1",
    wmode: 'transparent', //added so drop down menu + mobile menu do not hide behind video
    hd: "1"
  };
  var ytatts = {
    id: myytpid
...
(function($){
	Drupal.behaviors.ytp = {
		attach: function(context) {
      requirementsTxt = Drupal.settings.YTP.requirementsTxt;
      headerTxt = Drupal.settings.YTP.headerTxt;
      playTxt = Drupal.settings.YTP.playTxt;
      forwardTxt = Drupal.settings.YTP.forwardTxt;
      backTxt = Drupal.settings.YTP.backTxt;
      stopTxt = Drupal.settings.YTP.stopTxt;
      volUpTxt = Drupal.settings.YTP.volUpTxt;
      volDnTxt = Drupal.settings.YTP.volDnTxt;
      muteTxt = Drupal.settings.YTP.muteTxt;
      loopTxt = Drupal.settings.YTP.loopTxt;
      currentlyPlayingTxt = Drupal.settings.YTP.currentlyPlayingTxt;
      timeTxt = Drupal.settings.YTP.timeTxt;
      unmuteTxt = Drupal.settings.YTP.unmuteTxt;
      pauseTxt = Drupal.settings.YTP.pauseTxt;
      unloopTxt = Drupal.settings.YTP.unloopTxt;
      videoPlayListTxt = Drupal.settings.YTP.videoPlayListTxt;
      ofTxt = Drupal.settings.YTP.ofTxt;
